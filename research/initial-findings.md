...
- **Representation & Packaging**: [BIP 321](prior-art.md) supports the currently defined payment-instruction representation within a single URI or QR code.
...
The following behaviors are not found in the specifications surveyed so far and are left entirely to individual wallet applications:
...
- **Mempool-Responsive Selection**: Observations on whether wallets dynamically alter their selection defaults based on current network congestion and transaction fees (evaluated under hypothetical mempool scenarios).
...
- **No Representation Gap**: BIP 321 already supports the currently defined payment-instruction representation in a single payload. No new standard is needed to represent multi-method instructions.
